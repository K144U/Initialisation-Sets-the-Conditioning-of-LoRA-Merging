...
\midrule
Replication, step 0 (\S\ref{subsec:q1}) & \texttt{329d94a} &
  --- & \texttt{5857238} \\
Replication, $n=3$ (\S\ref{subsec:q1}) & \texttt{5792ff2} &
  --- & \texttt{d89e53a} \\
DARE with TIES (\S\ref{subsec:dare}) & \texttt{78f5399} &
  \texttt{ccf39f4}, \texttt{660ab63} & \texttt{7b08053} \\
Conditioning and the ridge (\S\ref{subsec:ridge}) & \texttt{3943a78} &
  \texttt{282c095}, \texttt{6b0a367} & \texttt{f877a85} \\
Rate exponent (\S\ref{subsec:w3}) & \texttt{3943a78} &
  \texttt{9718f96} (corrected) & \texttt{2e266a8} \\
\addlinespace
Solver replication (\S\ref{subsec:regmean}) & \texttt{44875ee} &
  \texttt{a7802f8}, \texttt{5a47b15} & \texttt{73a476d} \\
Untruncated and gate (\S\ref{subsec:untruncated}) & \texttt{44875ee} &
  \texttt{2a3ad03} & \texttt{43c8c00} \\
Repaired KnOTS (\S\ref{sec:discussion}) & \texttt{44875ee} &
  \texttt{3c840ca} & \texttt{368d7ff} \\
Merge matrix at $T=3$ (\S\ref{subsec:q1}) & \texttt{44875ee} &
  \texttt{67e0b89} & \texttt{19c32ff} \\
\addlinespace
Margin-aware control (\S\ref{subsec:q1}) & \texttt{44875ee} &
  \texttt{ce9099a} & \texttt{36efe56} \\
Shared arm at $n=3$ (\S\ref{subsec:shared-arm}) & \texttt{1f52ab1} &
  \texttt{dafe53a} & \texttt{36efe56} \\
Public-cohort prevalence (\S\ref{subsec:prevalence}) & \texttt{07740ff} &
  \texttt{fddec17} & \texttt{36efe56} \\
Downstream accuracy (\S\ref{subsec:downstream-cond}) & \texttt{7bfc8dd} &
  \texttt{b5e81ed} & \texttt{36efe56} \\
Heuristics null in accuracy (\S\ref{subsec:ridge}) & \texttt{3bbbbdb} &
  \texttt{7125fb3} & \texttt{5215020} \\
Drift across training (App.~\ref{subsec:drift-result}) & \texttt{8d3e468} &
  \texttt{09ccced} & \texttt{e48af8d} \\
\bottomrule
\end{tabular}
\end{table}
...
precede the step-0 result: \texttt{5792ff2} was committed after
\texttt{5857238} was read. That is the blindness limitation the amendment
...
ancestry check on \texttt{5792ff2} against \texttt{5857238} fails, and should.
...
\texttt{660ab63} is that repair, and it predates the verdict commit. The
...
\item \textbf{\texttt{prereg\_a1\_matrix}} (\texttt{329d94a}) fixes the
...
\item \textbf{\texttt{prereg\_a1\_matrix\_amendment}} (\texttt{5792ff2})
...
\item \textbf{\texttt{prereg\_dare\_ties}} (\texttt{78f5399}) asks whether
...
\item \textbf{\texttt{prereg\_conditioning}} (\texttt{3943a78}) governs the
...
\item \textbf{\texttt{prereg\_tmlr}} (\texttt{44875ee}) is the revision
...
\item \textbf{\texttt{prereg\_tmlr\_amendment}} (\texttt{a7802f8}) sets
...
\item \textbf{\texttt{prereg\_shared\_arm}} (\texttt{1f52ab1}) answers a
...
\item \textbf{\texttt{prereg\_prevalence}} (\texttt{07740ff}) fixes the
...
\item \textbf{\texttt{prereg\_downstream}} (\texttt{7bfc8dd}) asks whether
...
\item \textbf{\texttt{prereg\_downstream\_amendment}} (\texttt{2c18c8e})
...
\item \textbf{\texttt{prereg\_drift}} (\texttt{8d3e468}) discharges E3,
...
\item \textbf{\texttt{prereg\_heuristics\_downstream}} (\texttt{3bbbbdb})
...
\midrule
\texttt{prereg\_a1\_matrix} & \texttt{329d94a} & replication, step 0 \\
\texttt{prereg\_a1\_matrix\_amendment} & \texttt{5792ff2} & replication, $n=3$ \\
\texttt{prereg\_dare\_ties} & \texttt{78f5399} & DARE with TIES \\
\texttt{prereg\_conditioning} & \texttt{3943a78} & ridge sweep, rate exponent \\
\texttt{prereg\_tmlr} & \texttt{44875ee} & solver replication, gate, KnOTS, $T=3$ \\
\texttt{prereg\_tmlr\_amendment} & \texttt{a7802f8} & RegMean's minimal $\lambda$ \\
\texttt{prereg\_shared\_arm} & \texttt{1f52ab1} & the shared arm at $n=3$ \\
\texttt{prereg\_prevalence} & \texttt{07740ff} & public-cohort prevalence \\
\texttt{prereg\_downstream} & \texttt{7bfc8dd} & downstream accuracy \\
\texttt{prereg\_downstream\_amendment} & \texttt{2c18c8e} & HumanEval's 164 items \\
\texttt{prereg\_heuristics\_downstream} & \texttt{3bbbbdb} & the heuristics null in accuracy \\
\texttt{prereg\_drift} & \texttt{8d3e468} & how far $A$ travels (E3) \\
\bottomrule
\end{tabular}
\end{table}
