...
required); LoRM~\citep{lorm2024} extends it to low-rank adapters and
RegMean++~\citep{regmeanpp2025} refines the regularization. Ours differs
...
\S\ref{subsec:exp-rd-encoder} and sweep its ridge $\lambda$ in
Appendix~\ref{app:robustness} so the comparison does not conflate the
centroid with the regularizer.
...
every algorithm, and our measurements reconcile the two: real LoRA task
subspaces are linearly independent ($\deff = Tr$ in every layer of all
four bases), so subspace alignment has no overlap to exploit (we
measure KnOTS indistinguishable from Task Arithmetic) while
sign-level TIES helps. Where our ordering differs from theirs, the
