...
\item \textbf{Quadratic bridge holds on one of two bases.} E11
(\S\ref{subsec:exp-e11}) confirms that rd-encoder ridge approaches the
Fisher-quadratic optimum as adapter norms shrink on Yi-1.5-Chat
(ratio $\to 0.90$ at $\alpha = 0.1$). On Llama-3.1, rd-encoder
saturates the base-model floor at $\alpha = 0.1$ before the bridge
regime is reached, leaving Llama as ``unable to verify'' rather than
a refutation; a finer $\alpha$ scan between $0.10$ and $0.25$ would
close this caveat.
