...
\midrule
Replication, step 0 (\S\ref{subsec:q1}) & \texttt{5ad3d57} &
  --- & \texttt{fe23963} \\
Replication, $n=3$ (\S\ref{subsec:q1}) & \texttt{f8ebbc2} &
  --- & \texttt{b450fca} \\
DARE with TIES (\S\ref{subsec:dare}) & \texttt{a775ce7} &
  \texttt{7b0915b}, \texttt{f5c80e3} & \texttt{d4c62d4} \\
Conditioning and the ridge (\S\ref{subsec:ridge}) & \texttt{21da3a5} &
  \texttt{496ff49}, \texttt{89eef76} & \texttt{a13df0f} \\
Rate exponent (\S\ref{subsec:w3}) & \texttt{21da3a5} &
  \texttt{c4b80a8} (corrected) & \texttt{080d5e4} \\
\addlinespace
Solver replication (\S\ref{subsec:regmean}) & \texttt{81068b4} &
  \texttt{f179871}, \texttt{4e0ea62} & \texttt{8b6db0f} \\
Untruncated and gate (\S\ref{subsec:untruncated}) & \texttt{81068b4} &
  \texttt{0039652} & \texttt{34a571d} \\
Repaired KnOTS (\S\ref{sec:discussion}) & \texttt{81068b4} &
  \texttt{8c77e44} & \texttt{13d085f} \\
Merge matrix at $T=3$ (\S\ref{subsec:q1}) & \texttt{81068b4} &
  \texttt{a23b9f3} & \texttt{0b3abbf} \\
\addlinespace
Margin-aware control (\S\ref{subsec:q1}) & \texttt{81068b4} &
  \texttt{1fd37c3} & \texttt{a5d8e31} \\
Shared arm at $n=3$ (\S\ref{subsec:shared-arm}) & \texttt{c406f82} &
  \texttt{ea4ebb6} & \texttt{a5d8e31} \\
Public-cohort prevalence (\S\ref{subsec:prevalence}) & \texttt{a6fead2} &
  \texttt{6565f8c} & \texttt{a5d8e31} \\
Downstream accuracy (\S\ref{subsec:downstream-cond}) & \texttt{6abf41d} &
  \texttt{e43a6f7} & \texttt{a5d8e31} \\
Heuristics null in accuracy (\S\ref{subsec:ridge}) & \texttt{bea25cf} &
  \texttt{846d465} & \texttt{2190d9f} \\
Drift across training (App.~\ref{subsec:drift-result}) & \texttt{f327904} &
  \texttt{ef3b67f} & \texttt{b3052d4} \\
\bottomrule
\end{tabular}
\end{table}
...
precede the step-0 result: \texttt{f8ebbc2} was committed after
\texttt{fe23963} was read. That is the blindness limitation the amendment
...
ancestry check on \texttt{f8ebbc2} against \texttt{fe23963} fails, and should.
...
\texttt{f5c80e3} is that repair, and it predates the verdict commit. The
...
\item \textbf{\texttt{prereg\_a1\_matrix}} (\texttt{5ad3d57}) fixes the
...
\item \textbf{\texttt{prereg\_a1\_matrix\_amendment}} (\texttt{f8ebbc2})
...
\item \textbf{\texttt{prereg\_dare\_ties}} (\texttt{a775ce7}) asks whether
...
\item \textbf{\texttt{prereg\_conditioning}} (\texttt{21da3a5}) governs the
...
\item \textbf{\texttt{prereg\_tmlr}} (\texttt{81068b4}) is the revision
...
\item \textbf{\texttt{prereg\_tmlr\_amendment}} (\texttt{f179871}) sets
...
\item \textbf{\texttt{prereg\_shared\_arm}} (\texttt{c406f82}) answers a
...
\item \textbf{\texttt{prereg\_prevalence}} (\texttt{a6fead2}) fixes the
...
\item \textbf{\texttt{prereg\_downstream}} (\texttt{6abf41d}) asks whether
...
\item \textbf{\texttt{prereg\_downstream\_amendment}} (\texttt{77f6ade})
...
\item \textbf{\texttt{prereg\_drift}} (\texttt{f327904}) discharges E3,
...
\item \textbf{\texttt{prereg\_heuristics\_downstream}} (\texttt{bea25cf})
...
\midrule
\texttt{prereg\_a1\_matrix} & \texttt{5ad3d57} & replication, step 0 \\
\texttt{prereg\_a1\_matrix\_amendment} & \texttt{f8ebbc2} & replication, $n=3$ \\
\texttt{prereg\_dare\_ties} & \texttt{a775ce7} & DARE with TIES \\
\texttt{prereg\_conditioning} & \texttt{21da3a5} & ridge sweep, rate exponent \\
\texttt{prereg\_tmlr} & \texttt{81068b4} & solver replication, gate, KnOTS, $T=3$ \\
\texttt{prereg\_tmlr\_amendment} & \texttt{f179871} & RegMean's minimal $\lambda$ \\
\texttt{prereg\_shared\_arm} & \texttt{c406f82} & the shared arm at $n=3$ \\
\texttt{prereg\_prevalence} & \texttt{a6fead2} & public-cohort prevalence \\
\texttt{prereg\_downstream} & \texttt{6abf41d} & downstream accuracy \\
\texttt{prereg\_downstream\_amendment} & \texttt{77f6ade} & HumanEval's 164 items \\
\texttt{prereg\_heuristics\_downstream} & \texttt{bea25cf} & the heuristics null in accuracy \\
\texttt{prereg\_drift} & \texttt{f327904} & how far $A$ travels (E3) \\
\bottomrule
\end{tabular}
\end{table}
