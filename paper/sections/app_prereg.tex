...
Heuristics null in accuracy (\S\ref{subsec:ridge}) & \texttt{9cc2394} &
  \texttt{72a6bf9} & \texttt{77a7f37} \\
\bottomrule
\end{tabular}
\end{table}
...
Eleven documents govern the tests in this paper. Table~\ref{tab:prereg-docs}
lists them with the commit that introduced each, and the rules are
summarised below. \textbf{The documents themselves are in the supplementary
material, complete and unedited}, and also inside the anonymised git bundle
at the commits named here, which is the copy that cannot have been written
afterwards. Earlier versions of this paper reproduced them all verbatim in
the appendix; that is what the bundle is for, and printing them twice made
the paper longer without making anything more checkable.
...
altered across the eleven: the first document records the wall-clock time at
...
\item \textbf{\texttt{prereg\_a1\_matrix}} (\texttt{5693d9a}) fixes the
...
\item \textbf{\texttt{prereg\_a1\_matrix\_amendment}} (\texttt{44c0d39})
...
\item \textbf{\texttt{prereg\_dare\_ties}} (\texttt{0e0898d}) asks whether
...
\item \textbf{\texttt{prereg\_conditioning}} (\texttt{f48c4ef}) governs the
...
\item \textbf{\texttt{prereg\_tmlr}} (\texttt{1eabc27}) is the revision
...
\item \textbf{\texttt{prereg\_tmlr\_amendment}} (\texttt{0672c70}) sets
...
\item \textbf{\texttt{prereg\_shared\_arm}} (\texttt{41d8adc}) answers a
...
\item \textbf{\texttt{prereg\_prevalence}} (\texttt{5e8e736}) fixes the
...
\item \textbf{\texttt{prereg\_downstream}} (\texttt{8845efa}) asks whether
...
\item \textbf{\texttt{prereg\_downstream\_amendment}} (\texttt{9b20d1e})
...
\item \textbf{\texttt{prereg\_heuristics\_downstream}} (\texttt{9cc2394})
...
\centering
\small
\caption{The eleven pre-registration documents and the commit that introduced
...
\texttt{prereg\_heuristics\_downstream} & \texttt{9cc2394} & the heuristics null in accuracy \\
\bottomrule
\end{tabular}
\end{table}
