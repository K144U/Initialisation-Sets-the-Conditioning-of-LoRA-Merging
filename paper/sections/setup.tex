...
and decoder $D : \{1,\ldots,2^R\}\to\R^d$, both of which may
depend on $\boldsymbol H$ as shared side information: in any real
deployment $\boldsymbol H$ is determined by the LoRA factors
$U_t$ (with a curvature or Fisher surrogate for the spectra),
which both ends already hold, so only the $R$ bits describing
$\boldsymbol\tau$ are charged to the rate. The encoder observes
$\boldsymbol\tau$; the reconstruction is $w^\star :=
D(E(\boldsymbol\tau))$. Our lower bound holds even when
$\boldsymbol H$ is revealed for free to both ends. The \emph{rank-$r$
