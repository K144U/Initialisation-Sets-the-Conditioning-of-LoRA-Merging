...
\paragraph{Real-LLM setup.} The authors calibrate the bound against real LoRA
...
Yi-1.5-9B-Chat (MHA). For each the authors train $T=4$ adapters (rank $r = 16$
...
independent seeds. For a merged delta $w$ the authors measure per-task
...
no-quantisation rate, worst-task excess is strictly positive on every
...
The TVQ rate sweep is non-monotone on every base: $2$-bit quantisation
...
the five method-pair gaps behind the authors' recommendations leaves \emph{all
...
The authors instantiate the encoder of Theorem~\ref{thm:achv} with the projector
...
against untuned baselines would be an unfair win, the authors also sweep each
...
consistent across three matched seeds) and a tuned TIES ties it. The authors
...
Yi, the base where they independently flag NLL excess as an unreliable
proxy (R2). Part of the default-hyperparameter
Llama gap over RegMean ($0.243$) is the same sliver pathology that
breaks the authors' own $\lambda = 0$ construction ($0.340$): RegMean has no
...
To test whether the ordering survives beyond $T = 4$, the authors sweep
...
($R = 0.028$, inside) is ambiguous. The authors then \textbf{pre-registered} an
out-of-sample test (commit \texttt{564125e} in the public repository,
timestamped before the Mistral sweep ran): Mistral-7B, probe
...
below-window prediction is untested (no cohort the authors measured has
$R < 0.028$). The authors present the threshold as a calibrated heuristic with
...
deploys against, so the authors re-merge the $T = 4$ matrix and evaluate GSM8K
...
artifacts in $\lesssim 1$ CPU-minute per task; every cohort the authors
