...
\end{equation}
The authors linearize around the symmetric case $q_t \equiv 1/T$: let
...
$R^\star \approx 2/\sqrt{T} \approx 1$, vacuous. The authors therefore predict
...
\end{itemize}
The authors are careful \emph{not} to call this a validated bracket: the
\emph{below}-window prediction (TIES wins outright for $R < 0.025$) is
untested, since no cohort they measured has $R < 0.028$. What the three
...
should produce ambiguous TIES behavior. The authors pre-registered exactly this
test before the corresponding sweep ran (commit \texttt{564125e} in
the public repository), with operationalized criteria
...
ways at once. The authors therefore reproduce the mechanism on a controlled
dial with no base model. The authors synthesize $T = 7$ task vectors on a
...
against the consensus. The authors merge with the \emph{same} TIES code path
