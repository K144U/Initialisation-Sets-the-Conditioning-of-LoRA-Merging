...
independent initialisation. That $4.0$ is $T$, the identity for exactly
orthogonal subspaces, and recomputing at $T=3$ returns exactly $3.0$
(Appendix~\ref{app:geometry}). We record this as a correctness check on the
...
\paragraph{A gap we do not close.} There is a mismatch of objects running
through this paper and we flag it rather than let a reader find it.
Theorem~\ref{thm:general} constrains $w^\star \in \R^d$ with no rank
constraint. The interpolator $\tauH$ whose norm we report above has rank up
to $Tr = 64$. But our merge pipeline truncates every method back to rank
$r = 16$ for deployment parity, so the object whose norm we measure is not
one the pipeline can return, and the ridge sweep of \S\ref{subsec:ridge}
tests conditioning through a rank-$16$-constrained solver. Theory, geometry
and experiment therefore constrain three different objects. We expect the
conditioning story to survive, since truncation cannot reduce the ill-posedness
of the underlying system, but we have not shown that, and until the sweep is
repeated without truncation the link between the $\kappa$ we measure and the
degradation we observe is an inference rather than a measurement.
