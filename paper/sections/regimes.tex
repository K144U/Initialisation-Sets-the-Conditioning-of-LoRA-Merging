...
(\texttt{indep1/2/3}). The translation adapter does not learn its task; the authors
...
\eqref{eq:floor-exact}, and the authors do not claim to have done so. Computing
...
At $\mathrm{rtol} = 10^{-6}$ the authors obtain zero on all eight cohort-base pairs;
...
\paragraph{Is $\kappa(\bar H)$ a property of the cohort or of the authors'
surrogate?} $\bar H$ is built from the LoRA factors under the
specialisation $H_t = P_{V_t}$, so the question is fair and they answer it
...
spectrum. The authors therefore read $\kappa(\bar H)$ as a calibrated index of
...
no rank constraint. The interpolator $\tauH$ whose norm the authors report above has
...
constrains an unconstrained $w^\star$, and every merge the authors run is rank
...
\begin{itemize}[leftmargin=*,itemsep=1pt,topsep=3pt]
  ...
  \item Between the two --- one of the authors' $45$ public cohorts landed here and they
...
anyone else's adapters. So the authors ran the audit of \S\ref{subsec:audit} over public
...
\paragraph{Sampling frame, fixed in advance.} The authors walk the HuggingFace model
...
\textbf{no cohort was skipped after inspection}. The authors walked $1000$ models and
...
reported below. \textbf{Part B has not been run}, so the authors do not treat the
...
\texttt{init\_lora\_weights} is true, which is the default and what all the authors'
...
statement about published merging \emph{benchmarks}, and the authors make none: the
...
(Appendix~\ref{app:geometry}). The authors' cohorts are therefore $T=4$ nominal and
$3$ effective, and they describe them that way throughout.

\paragraph{The authors know why, and it is a data-pipeline defect rather than a
...
task or of LoRA. \textbf{The authors report it rather than repairing it}, because
