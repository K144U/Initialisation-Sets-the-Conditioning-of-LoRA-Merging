...
A concurrent submission measures a different consequence of the same
initialisation and arrives at the displacement of $A$ as the quantity that
would settle it. \citet{concurrentanon} report that entrywise sign
disagreement between LoRA adapters trained on \emph{different tasks} is
barely distinguishable from the disagreement between adapters that differ
only in seed, and propose that $A$ remaining near its random draw is why: if
the sign of an entry is set mostly by which random basis the run received,
then two runs agree on the update as a linear map while disagreeing
coordinate by coordinate about how to write it down. That paper names the
...
coincidence, which is why it appears here rather than among the related
work: the two papers are not independent tests of one
hypothesis, and the only thing exchanged between them is a measurement one of
them asked for.
