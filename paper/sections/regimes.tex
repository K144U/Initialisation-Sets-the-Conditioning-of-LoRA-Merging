...
to $A$ is proportional to $B$ and vanishes at initialisation, which is a
property of the parameterisation rather than a measurement: we did not
retain initial weights, so how far $A$ actually travelled is not something
this paper measures (Appendix~\ref{subsec:drift}). What we measure is the
consequence. And in most pipelines the initialisation
