...
achievability theorem (Theorem~\ref{thm:achievability}) instantiates
...
Theorem~\ref{thm:achievability} with the projector surrogate
...
Arithmetic baseline ($0.219$, Table~\ref{tab:exp-lora-full}) and
...
region of the quadratic surrogate in
Proposition~\ref{prop:quadratic-surrogate}, so even though the
surrogate distortion of $w^{*}$ is near zero (as
Theorem~\ref{thm:achievability} predicts), the actual
...
Theorem~\ref{thm:achievability}; the spec admits any positive
...
\paragraph{Cross-model generalization.} \textsc{TODO: 12-cell sweep
($\lambda \in \{0.05, 0.07, 0.10, 0.13\}$ on Mistral-7B,
Qwen-2.5-7B, Yi-1.5-9B) is running as of 2026-06-13. Fill
Table~\ref{tab:rd-ridge-xmodel} when complete. Pre-registered
decision rule (campaign notes):
\begin{itemize}
\item all three models show a U with minimum in $[0.05, 0.13]$ and
worst-task excess at the optimum below the corresponding TA
baseline $\Rightarrow$ the achievability salvage generalizes;
\item two of three hold $\Rightarrow$ the recipe is documented as
model-dependent with an outlier discussion;
\item zero or one hold $\Rightarrow$ the regularization-restored
construction is Llama-specific, and the dominant message of
\S\ref{sec:lower_bound}--\ref{sec:achievability} shifts to the
lower-bound contribution plus the spectral mechanism as a
diagnostic for when the achievability machinery applies.
\end{itemize}}

\paragraph{Discussion.} The Tikhonov regularization
$(\bar H + \lambda I)^{-1}$ is the same construction as
Theorem~\ref{thm:achievability} with a numerical-conditioning floor
