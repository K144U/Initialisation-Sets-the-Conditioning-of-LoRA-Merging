...
worst-task excess $0.497$, well above Task Arithmetic ($0.219$) and
TIES ($0.161$). Adding Tikhonov regularization with
$\lambda^\star = 0.05$ recovers worst-task excess $0.091$, the
...
a wide $\lambda$ band $[0.03, 0.20]$, with a sharp minimum at
$\lambda^\star = 0.05$.}
\label{fig:salvage}
\end{figure}
...
$\Phi^\star_{\lambda=0} = 0.497$ dominates the column). $\lambda =
0.05$ is the empirical optimum and beats Task Arithmetic ($0.219$)
and TIES ($0.161$). Figure~\ref{fig:salvage} plots the same sweep
