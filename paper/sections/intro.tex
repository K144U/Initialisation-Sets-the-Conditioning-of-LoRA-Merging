...
\paragraph{Contribution in one sentence.} \emph{We prove a
rate-distortion theorem that splits LoRA-merging worst-task error
into an irreducible task-overlap floor plus an exponentially-decaying
compression term, provide a regularized encoder
(\textsc{rd-encoder ridge}) that ties or beats every published method
at $T = 4$ and widens its margin at $T = 7$ on every base we test, and
convert the result into a five-rule decision tree for practitioners
that we validate across a $4$-base $\times$ $7$-method $\times$
$2$-downstream-metric matrix.}
