...
adapters (\S\ref{subsec:audit}). This is the part of the paper the authors expect
someone to use directly, and the rest of the paper is an argument that it
is worth running.

\paragraph{The effect splits cleanly by method.} The authors pre-registered the test
...
\paragraph{It reaches accuracy, not just the authors' metric.} A large difference
...
The authors report the principal cosines rather than $\kappa(\bar H)$ throughout,
...
$H_t = P_{V_t}$ adopted here (\S\ref{subsec:conditioning}).

\paragraph{Where this came from.} The authors arrived at the conditioning question
through a rate-distortion analysis of merging, which is in the paper because
it is the reason for looking at $\bar H$ at all and for no stronger reason than
...
been seen and the authors do not use it that way (\S\ref{subsec:withdrawn}). The rate exponent the bound predicts is unresolved:
the authors' sweep lacks the dynamic range to measure it (\S\ref{subsec:w3}). One of the authors' four adapters never learned its
...
under the authors' own names, titled \emph{A Rate-Distortion Function for Model
...
\item \textbf{A pre-registered demonstration that the collapse is invisible
  ...
  the authors' encoder and for RegMean, at $n = 3$ cohorts in \emph{both} regimes,
...
(\S\ref{subsec:dare}), which bounds an earlier claim of theirs rather than
adding a new one.
