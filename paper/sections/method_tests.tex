...
\paragraph{First, the null, and what it can actually detect.} The authors compare
...
\paragraph{The same null in a unit the authors do not disclaim.} Everything above is
...
with the lean present on both benchmarks. \textbf{The authors did not register a sign
...
their encoder, $\lambda^\star$ & 2.20--2.56 & 0.000 & 2.84--3.05 & 0.28 \\
\addlinespace
...
\paragraph{So the authors sweep the ridge, on two solvers.} To expose the
...
The other half of the prediction failed. The authors also predicted that $\lambda^\star$ itself would be at least ten times
...
pre-registered rules this makes the test \textsc{partial}, and the authors claim
...
\subsection{The same test on a solver the authors did not build}
...
$\lambda I$ is the authors', and the comparison below is between their regulariser
...
so they do not report a task-loss number for it. And $\alpha$ is a documented
...
\S\ref{subsec:conditioning} notes that the authors' pipeline truncates every merge
...
re-run. \textbf{Two entries here invite a second look and the authors have taken it.}
...
\subsection{A claim the authors withdraw}
...
as provisional. \textbf{The authors decline to reinstate it}, and the reason is
...
\paragraph{The negative result does not generalise, and the authors tested that.}
