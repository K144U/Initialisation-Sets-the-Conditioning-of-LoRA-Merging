...
remaining average-distortion subproblem via an $O(d)$-equivariance
argument and a rotationally-invariant Shannon bound
(\S\ref{subsec:main}).
...
\emph{Step 5 ($O(W)$-equivariance and Shannon bound).}
Conditioning on $W := \range(\bar H)$, the joint law of
$(U_t, \tau_t)_{t=1}^T$ is $O(W)$-equivariant: for any $\tilde O
\in O(W)$ extended to $O(d)$ by the identity on $W^\perp$,
$\tau_t\mapsto O\tau_t$ and $H_t\mapsto OH_tO^\top$, leaving
the marginal joint law unchanged. The pushforward
$\eta\mapsto O\eta$ is therefore $O(W)$-invariant in the
conditional law. A rotationally-invariant rate-$R$
quantization bound — a rank-$r$ analog of TurboQuant
Theorem~3~\cite{zandieh2025turboquant} — gives
\begin{equation*}
  \E\bigl[\|\bar H^{1/2}w^\star-\eta\|^2\,\bigm|\,W\bigr]
  \;\geq\;
  c_{\mathrm{TQ}}\cdot\E\bigl[\|\eta\|^2\,\bigm|\,W\bigr]\cdot
  2^{-2R/\deff}.
\end{equation*}
The key $D_t$-freeness of the argument survives in this step
because $v_t = B D_t^{-1/2} u_t$ lives in $\R^r$ and is
$O(d)$-free regardless of which $D_t$ it uses.

\emph{Step 6 (combine).} Taking expectation over $W$ and using
