...
export ORCH_STATE=orchestrator_state_ridge_cond.json
...
for C in seed1 indep1; do
  ...
  if [ "$N" -lt 16 ]; then
    echo "[rc] ABORT: $C has $N/16 adapters"
    exit 2
  fi
  echo "[rc] $C $N/16 adapters present"
done

MANIFEST="${MANIFEST:-code/phase3/configs/ridge_cond_manifest.json}"
echo "[$(date '+%F %T')] rdm_rc job=$PBS_JOBID manifest=$MANIFEST gpus=$GPUS host=$(hostname)"
