#!/bin/bash
# Keeper for the heuristics-downstream run (80 cells).
#
...
#
# Stop it with:  touch _KEEPER_HDS_STOP
...
TARGET=80
RESDIR=results/phase3/eval_heur_downstream
MAX_RESUBMITS=8
n_resub=0
last_done=-1
stalls=0
...
while true; do
  ...
  if [ "$done_n" -ge "$TARGET" ]; then
    ...
    echo "  Rules: notes/prereg_heuristics_downstream_2026-08-16.md (8ff7fb4)."
    echo "  Now run: python code/phase3/scripts/analyze_heuristics_downstream.py"
