...
# E6 pilot stage 1 — train 4 new yi-1.5-9B adapters at r=16 on
# MATH / CodeAlpaca / Dolly / XSum (the "easy-format" task subset
# that works with the default prompt_field/answer_field path in
# data_loaders.py). After completion the merge-eval orchestrator
# (pbs_orchestrator_e6_pilot_eval.sh) runs across T in {2,4,8}.
...
export ORCH_SENTINEL=_E6_PILOT_TRAIN_COMPLETE
export ORCH_STATE=orchestrator_state_e6_pilot_train.json

MANIFEST="${MANIFEST:-code/phase3/configs/e6_pilot_train_manifest.json}"
...
echo "[$(date '+%F %T')] rdm_e6tr job=$PBS_JOBID manifest=$MANIFEST gpus=$GPUS"
