...
paper. All ten pre-registrations are reproduced verbatim in
\S\ref{subsec:prereg-docs}, included from the source files at build time
rather than retyped, with a single timezone abbreviation redacted for
anonymity and marked where it occurs. Each was committed to version control
...
An earlier version went on to say that we did not \emph{believe} the
overlap affected comparisons between methods. That belief is now tested.
