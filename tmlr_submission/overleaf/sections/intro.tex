...
explains a good deal of that instability, and that can be measured before
...
shared. The fifteen have a mechanism: each of these methods returns a
bounded-norm near-mean solution that never enters the ill-conditioned
directions, and its energy there is a rounding error. Of the five cells
...
through a rate-distortion analysis, and the analysis is in the paper
because it is the reason we looked at $\bar H$ at all. We treat merging as
lossy compression: $T$ task vectors summarised by one weight vector, with
the figure of merit the \emph{worst-task} excess loss, since a merged model
that collapses on a single task is not deployable. \S\ref{sec:lower_bound}
proves a lower bound holding for every merging algorithm, even one given
the curvature data free,
\begin{equation*}
...
We report it as provenance and not as support, for three reasons given in
full in \S\ref{sec:discussion}. Its floor term is exactly zero on all four
base models under every initialisation we tried, so it carries no
operational content for LoRA merging as practised
(\S\ref{subsec:floor-zero}). Its rate term rests on an assumption we state
and do not prove (Assumption~\ref{asm:epr}), and our attempt to measure the
predicted exponent has too little dynamic range to do so
(\S\ref{subsec:w3}). And Theorem~\ref{thm:general} constrains a $w^\star$
carrying no rank constraint, while every merge we run is truncated to rank
$r$ (\S\ref{subsec:conditioning}). Everything in \S\ref{sec:regimes} and
...
initialisation geometry; a second control, registered afterwards and
identified as secondary, reinstates it only as provisional
(\S\ref{subsec:q1}). A rate-exponent falsification we previously reported
rested on an asymptote computed at a different ridge value and is
withdrawn, and the refitted sweep lacks the range to measure the exponent
at all (\S\ref{subsec:w3}). One of our four adapters never learned its
task, so our cohorts are $T=4$ nominal and $3$ effective, and we report
every geometric quantity at both. All ten pre-registrations are reproduced
in Appendix~\ref{app:prereg} with the commit ordering that makes them
checkable.
