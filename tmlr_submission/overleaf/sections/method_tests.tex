...
benchmarks. That precondition is \emph{partly} discharged.
\S\ref{subsec:prevalence} shows the regime in $37.8\%$ of $45$ public
cohorts, which is the prevalence half. The half that would bear directly on
published benchmarks, auditing the cohorts those benchmarks actually
release, is registered as Part B of that document and has not been run. So
the reinstatement stays provisional on both counts, and we make no claim
that any specific published evaluation is affected.
