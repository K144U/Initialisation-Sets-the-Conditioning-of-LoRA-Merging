...
\citet{concurrentanon} measures a different consequence of the same
initialisation and arrives at the displacement of $A$ as the quantity that
would settle it. It reports that entrywise sign disagreement between LoRA
adapters trained on \emph{different tasks} is barely distinguishable from the
disagreement between adapters that differ only in seed, and proposes that $A$
remaining near its random draw is why: if the sign of an entry is set mostly
by which random basis the run received, then two runs agree on the update as a
linear map while disagreeing coordinate by coordinate about how to write it
down. It names the displacement measurement as its natural next experiment and
does not run it. Appendix~\ref{subsec:drift-result} runs it. We draw no
support from the agreement: the two are not independent tests of one
