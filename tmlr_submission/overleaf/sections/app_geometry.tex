...
\textbf{Not measured: the distance each adapter travelled from its own
initialisation,} $\|A - A_0\|_F/\|A_0\|_F$. This needs $A_0$, and we did not
retain the initial weights or any intermediate checkpoint. No script in our
pipeline computes it and no result file contains it, which we verified
directly rather than by recollection.

\textbf{An earlier version of this work conflated the two}, quoting the
measured $0.16$ to $0.19$ and describing it as the distance each adapter had
moved from its initialisation. The number is correct and is the
shared-cohort entry of the column above; the description of what it measures
was wrong. We report the error here rather than silently relabelling it,
because the two readings support different arguments: a small distance
\emph{from initialisation} would say training barely moved $A$, which is the
mechanism of \S\ref{subsec:mechanism}, whereas a small distance
\emph{between adapters} says only that they ended up near each other, which
is the phenomenon. We have the second and not the first.

This has a registered consequence and we apply it. The E3 rule in
\S\ref{subsec:prereg-docs} required $\|\Delta A\|/\|A\|$ at $25$, $50$, $75$
and $100\%$ of training steps, and scopes every geometry claim to ``at this
training budget'' if the rule cannot be evaluated. It cannot: the
checkpoints do not exist. \textbf{So every geometry claim in this paper is
scoped to this training budget}, and we do not claim that a longer or
shorter run would show the same subspace collapse.
...
have been a useful corroboration of why; it is not load-bearing for what.
