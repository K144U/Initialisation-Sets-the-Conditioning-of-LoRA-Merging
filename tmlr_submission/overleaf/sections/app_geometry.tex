...
$\geq 0.9947$, soft $\deff \approx 16.3$, and $0.745$ in the fourth column.
...
cohort and $0.745$ on the shared one. The independent value is not a
coincidence and is worth reading as a check on the instrument: for two
independent draws of equal norm, $\|A_i - A_j\|^2 \approx \|A_i\|^2 +
\|A_j\|^2$, so the ratio must be about $\sqrt 2 = 1.414$. It is. The shared
cohort sits at roughly half that, which is the signature the paper is about,
and \S\ref{subsec:prevalence} finds the same two populations in public
adapters.
...
retain the initial weights or any intermediate checkpoint. An earlier
version of this work quoted $0.16$ to $0.19$ for this quantity on the shared
cohort. We withdraw that figure: it does not come from any measurement in
this paper's pipeline, which computes only the between-adapter distance
above, and we could not reproduce its provenance.
