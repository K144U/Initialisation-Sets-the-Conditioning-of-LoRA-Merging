...
\textbf{[BUILD NOTE, REMOVE BEFORE SUBMISSION: the last four rows currently
carry working-repository hashes. The bundle must be rebuilt from the final
history and every hash in this table regenerated from the mapping
\texttt{build\_anon\_bundle.sh} prints.]}}
\label{tab:commits}
\begin{tabular}{llll}
\toprule
test & rules & analyzer or implementation & result \\
\midrule
Replication, step 0 (\S\ref{subsec:q1}) & \texttt{0d9924b} &
  --- & \texttt{175453f} \\
Replication, $n=3$ (\S\ref{subsec:q1}) & \texttt{d503347} &
  --- & \texttt{7edf68a} \\
DARE with TIES (\S\ref{subsec:dare}) & \texttt{100cd43} &
  \texttt{b94331c}, \texttt{672a72c} & \texttt{3a0822e} \\
Conditioning and the ridge (\S\ref{subsec:ridge}) & \texttt{fa5f3c3} &
  \texttt{6e51df3}, \texttt{de2aa10} & \texttt{25d7de0} \\
Rate exponent (\S\ref{subsec:w3}) & \texttt{fa5f3c3} &
  \texttt{40eae48} (corrected) & \texttt{dc2991a} \\
\addlinespace
Solver replication (\S\ref{subsec:regmean}) & \texttt{0c8b9e8} &
  \texttt{c597e31}, \texttt{2a49b3e} & \texttt{a5e1ea1} \\
Untruncated and gate (\S\ref{subsec:untruncated}) & \texttt{0c8b9e8} &
  \texttt{fb21c0f} & \texttt{23c251b} \\
Repaired KnOTS (\S\ref{sec:discussion}) & \texttt{0c8b9e8} &
  \texttt{e8c968a} & \texttt{30d2c04} \\
Merge matrix at $T=3$ (\S\ref{subsec:q1}) & \texttt{0c8b9e8} &
  \texttt{7a12e2c} & \texttt{0d35789} \\
\addlinespace
Margin-aware control (\S\ref{subsec:q1}) & \texttt{0c8b9e8} &
  \texttt{ae3fdf1} & \texttt{f20b13c} \\
Shared arm at $n=3$ (\S\ref{subsec:shared-arm}) & \texttt{603e013} &
  \texttt{778da6e} & \texttt{f20b13c} \\
Public-cohort prevalence (\S\ref{subsec:prevalence}) & \texttt{84526b9} &
  \texttt{11bc8a7} & \texttt{f20b13c} \\
Downstream accuracy (\S\ref{subsec:downstream-cond}) & \texttt{df832fe} &
  \texttt{e66d273} & \texttt{f20b13c} \\
\bottomrule
\end{tabular}
\end{table}
...
precede the step-0 result: \texttt{d503347} was committed after
\texttt{175453f} was read. That is the blindness limitation the amendment
...
ancestry check on \texttt{d503347} against \texttt{175453f} fails, and should.
...
\texttt{672a72c} is that repair, and it predates the verdict commit. The
...
\midrule
\texttt{prereg\_a1\_matrix} & \texttt{0d9924b} & replication, step 0 \\
\texttt{prereg\_a1\_matrix\_amendment} & \texttt{d503347} & replication, $n=3$ \\
\texttt{prereg\_dare\_ties} & \texttt{100cd43} & DARE with TIES \\
\texttt{prereg\_conditioning} & \texttt{fa5f3c3} & ridge sweep, rate exponent \\
\texttt{prereg\_tmlr} & \texttt{0c8b9e8} & solver replication, gate, KnOTS, $T=3$ \\
\texttt{prereg\_tmlr\_amendment} & \texttt{c597e31} & RegMean's minimal $\lambda$ \\
\texttt{prereg\_shared\_arm} & \texttt{603e013} & the shared arm at $n=3$ \\
\texttt{prereg\_prevalence} & \texttt{84526b9} & public-cohort prevalence \\
\texttt{prereg\_downstream} & \texttt{df832fe} & downstream accuracy \\
\texttt{prereg\_downstream\_amendment} & \texttt{4d48987} & HumanEval's 164 items \\
\bottomrule
\end{tabular}
\end{table}
